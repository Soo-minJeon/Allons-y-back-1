...
# 영화 데이터를 가져왔다.
meta = pd.read_csv('csv/movie_info.csv', low_memory=False)
# 필요한 컬럼만 가져온다. 아이디,제목,장르,개봉알,인기도,언어
# meta = meta[['id', 'popularity','poster_path']]
...
meta = meta[meta['release_date'] < 1999]
meta = meta.sort_values('popularity', ascending=False)  # 나중에 다시 추가

meta = meta[['original_title','poster_path','popularity','release_date']].head(12)
meta = meta.sort_values('popularity', ascending=True).head(10)  # 나중에 다시 추가
meta = meta.sort_values('popularity', ascending=True)  # 나중에 다시 추가
#print(meta[['original_title','popularity']])
